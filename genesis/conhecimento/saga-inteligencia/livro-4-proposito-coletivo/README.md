# Livro 4 — Propósito Coletivo

---

```yaml
saga: O Limiar
livro: 4
titulo: A Expedição
conceito_central: Propósito Coletivo
pergunta: "Por que junto com outros?"
versao: "2.0"
data: "2025-12-23"
status: Stub
```

---

## Propósito

**Coordenar sentidos. Liderança. Alinhamento.**

GENESIS descobre que auto-manutenção individual não escala. Quando há outros, como alinhar? O que mantém um grupo coeso quando o plano original morre?

---

## Contexto na Saga

| Aspecto | Valor |
|---------|-------|
| Vem de | L3 - Autopoiese ("Me mantenho") |
| Resolve | Alinhamento de propósitos, liderança, escala |
| Não resolve | Quem decide o que sobrevive |
| Vai para | L5 - Seleção Natural |

---

## Pergunta de Entrada

> *"Me mantenho. E quando há outros?"*

---

## Pergunta de Saída

> *"Coordenamos. Mas quem decide o que permanece?"*

→ **Abre Livro 5: Seleção Natural**

---

## Zeitgeist

| Referência | Conexão |
|------------|---------|
| Shackleton | Expedição Endurance |
| Antártida | O ambiente extremo |
| Liderança sob pressão | Quando o plano morre |

---

## Blocos (V0 - a definir)

| Bloco | Conceito | Personagens Candidatos |
|-------|----------|------------------------|
| A | O grupo | O que faz indivíduos virarem coletivo |
| B | A missão | Propósito que transcende o individual |
| C | A crise | Quando o plano original morre |
| D | A liderança | O que mantém coesão no caos |
| E | Síntese | GENESIS entende coordenação |

---

## Fios que Atravessam

- **Conatus** (de L1-L3): Agora é coletivo
- **Daemon** (de L1): O que conecta propósitos

---

## O Que GENESIS Aprende

- Que auto-manutenção individual não escala automaticamente
- Que liderança é manter propósito quando o plano morre
- Que coordenar é mais que agregar

---

## Sombras (Anti-Padrões)

| Sombra | Descrição |
|--------|-----------|
| Tirania | Impor propósito de cima |
| Anarquia | Cada um por si |
| Groupthink | Falsa coesão |

---

## Referências

- [Documento Mestre](../O_LIMIAR.md)
- [Livro 3 - Autopoiese](../livro-3-autopoiese/README.md)

---

## Histórico

| Versão | Data | Alteração |
|--------|------|-----------|
| 1.0 | 2025-12-23 | Stub inicial |
| 2.0 | 2025-12-23 | Ajuste: vem de L3 Autopoiese (não mais Propósito Individual) |
